...
export default {
  ...
  methods: {
    ...
    initScroll() {
      ...
      //固定的高  包裹器
      const scrollTop1 = document.querySelector(".allListWrap");
      console.log(scrollTop1);
      //ul的高
      const allListHeight = document.querySelector(".allList");
      console.log(allListHeight);
      // const scrollY =  this.scrollY = allListHeight.scrollHeight-scrollTop1.scrollHeight
      // console.log(scrollY,'滑动的距离' )
      // scrollTop1.style.height = viewHeight+'px'

      this.$nextTick(() => {
        this.allListWrap = new BScroll(".allListWrap", {
          // click: true,
          scrollY: true,
          probeType: 3,
          // 忽略竖直方向的滚动
        ...
        this.allListWrap.on("scroll", ({y}) => {
          // console.log('2222',y);
          //滑动过程中的位置
          this.scrollY = Math.abs(y);
        });
        this.allListWrap.on("scrollEnd", ({ y }) => {
          console.log("滑动ul最后停的位置", y);
          //滑动ul最后停的位置
          this.scrollY = Math.abs(y);
             
        });
         this.allListWrap.on("touchEnd", ({ y }) => {
          console.log('手指或者鼠标离开的位置',  y);
          //手指或者鼠标离开的位置
          this.scrollY = Math.abs(y);
